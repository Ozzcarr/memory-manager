...
void test_memory_overcommit() {
    ...
    my_assert(block2 ==
              NULL);  // Expect NULL because it exceeds available memory
...
void test_exact_fit_reuse() {
    ...
    my_assert(block1 ==
              block2);  // Should be the same address if reused properly
...
void test_memory_reuse() {
    ...
    mem_free(block1);
    void *block3 =
        mem_alloc(128);  // This should ideally reuse the space from block1
...
void test_block_merging() {
    ...
    mem_free(block1);
    mem_free(block3);
    mem_free(block2);  // Freeing block2 should trigger merging with block1 and
                       // block3
...
void test_non_contiguous_allocation_failure() {
    ...
    mem_free(
        block3);  // Free the third block, leaving non-contiguous free slots

    // Attempt to allocate a block larger than any single free block but smaller
    // than the total free space
    void *block4 = mem_alloc(500);
    my_assert(
        block4 ==
        NULL);  // This allocation should fail due to lack of contiguous space
...
void test_contiguous_allocation_success() {
    ...
    mem_free(
        block1);  // Free block1 and block2 to create a contiguous free space
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        printf(
            " 2. test_alloc_and_free - Test basic allocation and "
            "deallocation\n");
        printf(" 3. test_resize - Test resizing allocated memory\n");

        printf("\nStress and Edge Cases:\n");
        printf(
            " 4. test_exceed_single_allocation - Test allocation beyond total "
            "memory\n");
        printf(
            " 5. test_exceed_cumulative_allocation - Test cumulative "
            "allocations exceeding total memory\n");
        ...
        printf(
            " 9. test_double_free - Test handling of double free operations\n");
        printf(
            " 10. test_memory_fragmentation - Test handling of memory "
            "fragmentation\n");
        printf(
            " 11. test_edge_case_allocations - Test allocations at edge "
            "conditions\n");

        printf("\nAdvanced Memory Management:\n");
        printf(
            " 12. test_frequent_small_allocations - Test frequent small "
            "allocations\n");
        printf(" 13. test_memory_reuse - Test reuse of freed memory\n");
        printf(
            " 14. test_block_merging - Test merging of adjacent free blocks\n");
        printf(
            " 15. test_non_contiguous_allocation_failure - Ensure failure when "
            "no contiguous block fits\n");
        printf(
